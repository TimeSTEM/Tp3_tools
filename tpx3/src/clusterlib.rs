...
pub mod cluster {
    ...
    impl SingleElectron {
        ...
        //uses the corrected time value
        pub fn relative_corrected_time_from_coincident_photon(&self) -> Option<i64> {
            let reference_time = self.coincident_photon()?.time();
            Some((self.corrected_time()?) as i64 - reference_time as i64)
        }
