...
fn main() -> Result<(), Tp3ErrorKind> {
    ...
    ***Instructions***:
    ...
    Turn the script using the --release tag. At least a single argument must be parsed, which is the folder containing your .tpx3 data. Data must be accompanied with a json file, with matching names.
    Example:

    >>  cargo run --bin time-resolved --release 'C:\\Users\\SpongeBob\\Data'
    
    >>  cargo run --bin time-resolved --release 'C:\\Users\\SpongeBob\\Data' 1
    ...
    For this particular script:
        ...
        -> Cluster correction can be activated by parsing a second argument, varying from 0 to 5. There are three automatic cluster correction:
            o '0' => No correction;
            o '1' => Average;
            o '2' => Maximum ToT;
        ...
        -> The time delay and time width are defined at compile-time, so you should change them at constlib.rs instead;
