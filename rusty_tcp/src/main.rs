...
fn connect_and_loop() {
    ...
    match my_settings.mode {
        0 => {
            ...
            modes::build_spectrum(pack_sock, vec_ns_sock, my_settings, frame_tdc, none_tdc);
        },
        1 => {
            let frame_tdc = PeriodicTdcRef::new(TdcType::TdcOneRisingEdge, &mut pack_sock);
            let laser_tdc = PeriodicTdcRef::new(TdcType::TdcTwoFallingEdge, &mut pack_sock);
     
            modes::build_spectrum(pack_sock, vec_ns_sock, my_settings, frame_tdc, laser_tdc);
        },
        2 => {
            ...
            modes::build_spim(pack_sock, vec_ns_sock, my_settings, spim_tdc, np_tdc);
        },
        3 => {
            let spim_tdc = PeriodicTdcRef::new(TdcType::TdcOneFallingEdge, &mut pack_sock);
            let laser_tdc = PeriodicTdcRef::new(TdcType::TdcTwoFallingEdge, &mut pack_sock);
            
            modes::build_spim(pack_sock, vec_ns_sock, my_settings, spim_tdc, laser_tdc);
        },
        4 => {
            let spim_tdc = PeriodicTdcRef::new(TdcType::TdcOneFallingEdge, &mut pack_sock);
            let pmt_tdc = NonPeriodicTdcRefMonitor::new(TdcType::TdcTwoFallingEdge, &mut pack_sock);
            
            modes::build_spim(pack_sock, vec_ns_sock, my_settings, spim_tdc, pmt_tdc);
        },
